...
if(isset($_POST['form_embroidery'])) {
    
    $valid = 1;

    if(empty($_POST['embroidery_title'])) {
        $valid = 0;
        $error_message .= 'Title can not be empty<br>';
    }

    if(empty($_POST['embroidery_content'])) {
        $valid = 0;
        $error_message .= 'Content can not be empty<br>';
    }

    $path = $_FILES['embroidery_banner']['name'];
    $path_tmp = $_FILES['embroidery_banner']['tmp_name'];
    ...
    if($valid == 1) {
        ...
        if($path != '') {
            ...
            foreach ($result as $row) {
                ...
                unlink('../assets/uploads/'.$embroidery_banner);
            }

            // updating the data
            $final_name = 'embroidery-banner'.'.'.$ext;
            move_uploaded_file( $path_tmp, '../assets/uploads/'.$final_name );

            // updating the database
            $statement = $pdo->prepare("UPDATE tbl_page SET embroidery_title=?,embroidery_content=?,embroidery_banner=?,embroidery_meta_title=?,embroidery_meta_keyword=?,embroidery_meta_description=? WHERE id=1");
            $statement->execute(array($_POST['embroidery_title'],$_POST['embroidery_content'],$final_name,$_POST['embroidery_meta_title'],$_POST['embroidery_meta_keyword'],$_POST['embroidery_meta_description']));
        } else {
            // updating the database
            $statement = $pdo->prepare("UPDATE tbl_page SET embroidery_title=?,embroidery_content=?,embroidery_meta_title=?,embroidery_meta_keyword=?,embroidery_meta_description=? WHERE id=1");
            $statement->execute(array($_POST['embroidery_title'],$_POST['embroidery_content'],$_POST['embroidery_meta_title'],$_POST['embroidery_meta_keyword'],$_POST['embroidery_meta_description']));
        }

        $success_message = 'Embroidery Page Information is updated successfully.';
...
foreach ($result as $row) {
    ...
    $embroidery_banner = $row['embroidery_banner'];
    $embroidery_meta_title = $row['embroidery_meta_title'];
    $embroidery_meta_keyword = $row['embroidery_meta_keyword'];
    $embroidery_meta_description = $row['embroidery_meta_description'];
        ...
        <div class="col-md-12">
                ...
                <div class="nav-tabs-custom">
                    <ul class="nav nav-tabs">
                        <li class="active"><a href="#tab_1" data-toggle="tab">About Toda</a></li>
                        <li><a href="#tab_5" data-toggle="tab">About Toda Embroidery</a></li>
                            ...
                            <div class="box box-info">
                                <div class="box-body">
                                    <div class="form-group">
                                        <label for="" class="col-sm-3 control-label">Page Title * </label>
                                        <div class="col-sm-5">
                                            <input class="form-control" type="text" name="embroidery_title" value="<?php echo $embroidery_title; ?>">
                                        </div>
                                    </div>
                                    <div class="form-group">
                                        <label for="" class="col-sm-3 control-label">Page Content * </label>
                                        <div class="col-sm-8">
                                            <textarea class="form-control" name="embroidery_content" id="editor2"><?php echo $embroidery_content; ?></textarea>
                                        </div>
                                    </div>
                                    <div class="form-group">
                                        <label for="" class="col-sm-3 control-label">Existing Banner Photo</label>
                                        <div class="col-sm-6" style="padding-top:6px;">
                                            <img src="../assets/uploads/<?php echo $embroidery_banner; ?>" class="existing-photo" style="height:80px;">
                                        </div>
                                    </div>
                                    <div class="form-group">
                                        <label for="" class="col-sm-3 control-label">New Banner Photo</label>
                                        <div class="col-sm-6" style="padding-top:6px;">
                                            <input type="file" name="embroidery_banner">
                                        </div>
                                    </div>
                                    <div class="form-group">
                                        <label for="" class="col-sm-3 control-label">Meta Title</label>
                                        <div class="col-sm-8">
                                            <input class="form-control" type="text" name="embroidery_meta_title" value="<?php echo $embroidery_meta_title; ?>">
                                        </div>
                                    </div>             
                                    <div class="form-group">
                                        <label for="" class="col-sm-3 control-label">Meta Keyword </label>
                                        <div class="col-sm-8">
                                            <textarea class="form-control" name="embroidery_meta_keyword" style="height:100px;"><?php echo $embroidery_meta_keyword; ?></textarea>
                                        </div>
                                    </div>
                                    <div class="form-group">
                                        <label for="" class="col-sm-3 control-label">Meta Description </label>
                                        <div class="col-sm-8">
                                            <textarea class="form-control" name="embroidery_meta_description" style="height:100px;"><?php echo $embroidery_meta_description; ?></textarea>
                                        </div>
                                    </div>                                    
                                    <div class="form-group">
                                        <label for="" class="col-sm-3 control-label"></label>
                                        <div class="col-sm-6">
                                            <button type="submit" class="btn btn-success pull-left" name="form_embroidery">Update</button>
                                        </div>
                                    </div>
                                </div>
                            </div>
